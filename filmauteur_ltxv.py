    ...
    def INPUT_TYPES(cls):
        ...
        return {
            "required": {
                ...
                "bypass_img_ref": ("BOOLEAN", {"default": False, "tooltip": "Ignores all reference images"}),
                "bypass_first_frame": ("BOOLEAN", {"default": False, "tooltip": "Ignores the Image-to-Video scene input"}),
                "load_audio_from_file": ("BOOLEAN", {"default": False, "tooltip": "Encodes the audio_input instead of generating LTXV native audio."}),
                "bypass_audio_ref": ("BOOLEAN", {"default": False, "tooltip": "Ignores the audio_ref input."}),
                "image_ref_str": ("FLOAT", {"default": 1.0, "min": 0.0, "max": 3.0, "step": 0.05, "tooltip": "Strength of the image_ref batch. Values over 1.0 may cause artifacts or burning."}),
                "first_frame_str": ("FLOAT", {"default": 1.0, "min": 0.0, "max": 1.0, "step": 0.05}),
                "identity_guidance_scale": ("FLOAT", {"default": 3.0, "min": 0.0, "max": 100.0, "step": 0.01, "tooltip": "Strength of identity guidance for ID-LoRA."}),
                
                # --- GROUP: Enhance ---
                "grp_ollama_enhance": (["▼ Enhance"], {}),
                "use_ollama": ("BOOLEAN", {"default": False, "tooltip": "Use local Ollama to visually describe inputs and revamp the prompt."}),
                ...
                "length_in_seconds": ("FLOAT", {"default": 5.0, "min": 0.1, "max": 300.0, "step": 0.1, "tooltip": "Total video length in seconds. Note: length_in_seconds must be evenly divisible by total number of shots."}),
                "frame_rate": ("FLOAT", {"default": 24.0, "min": 1.0, "max": 120.0, "step": 1.0}),
                
                # --- GROUP: Sampling ---
                "grp_sampling": (["▼ Sampling"], {}),
                "sampling_stages": ("INT", {"default": 2, "min": 1, "max": 3, "tooltip": "Number of processing stages. 1 = No upscale, 2 = One 2x upscale pass, 3 = Two 2x upscale passes (4x total)."}),
                ...
                "primary_steps": ("STRING", {"multiline": False, "default": "1.0, 0.995, 0.99, 0.9875, 0.975, 0.65, 0.28, 0.07, 0.0", "tooltip": "Enter a single number for steps (e.g., 20), or a comma-separated list for manual sigmas (e.g., 1.0, 0.8, 0.0)."}),
                ...
                "eta": ("FLOAT", {"default": 0.95, "min": -100.0, "max": 100.0, "step": 0.01, "round": False, "tooltip": "Calculated noise amount to be added, then removed, after each step."}),
                "bongmath": ("BOOLEAN", {"default": True, "tooltip": "Injects BONGMATH parameter into extra_options to act exactly as ClownSampler does for RES4LYF nodes."}),
                "autoregressive_chunking": ("BOOLEAN", {"default": True, "tooltip": "Automatically flush VRAM and outpaint the video in chunks if the length exceeds the chunk size."}),
                "chunk_size_seconds": ("FLOAT", {"default": 60.0, "min": 5.0, "max": 300.0, "step": 1.0, "tooltip": "The max duration (in seconds) generated in a single pass before flushing VRAM."}),
                "context_window_seconds": ("FLOAT", {"default": 10.0, "min": 0.0, "max": 300.0, "step": 1.0, "tooltip": "Seconds of previous video the model can 'see'. Caps render time! Set equal to chunk_size to keep rendering times perfectly flat."}),
                
                # --- GROUP: Refinement ---
                "grp_refinement": (["▼ Refinement"], {}),
                "temporal_upscale": ("BOOLEAN", {"default": False, "tooltip": "Triggers the temporal upscaler on or off (use to double the input frame rate, thus doubling the frame count, and refine the final video, cleaning up artifacts)."}),
                "restore_faces": ("BOOLEAN", {"default": False, "tooltip": "Apply CodeFormer face restoration to all frames. Requires a valid model selected below."}),
                "facerestore_model": (get_trixope_facerestore_models(), {"tooltip": "Select the CodeFormer Face Restore Model."}),
                "facedetection": (["retinaface_resnet50", "retinaface_mobile0.25", "YOLOv5l", "YOLOv5n"], {"tooltip": "Face detection model."}),
                "codeformer_fidelity": ("FLOAT", {"default": 0.5, "min": 0.0, "max": 1.0, "step": 0.05, "tooltip": "Balance between quality and identity. 0 is high quality, 1 is high fidelity."}),
                "face_restore_color_match": ("BOOLEAN", {"default": True, "tooltip": "Automatically match the hue, saturation, and luminance of the restored face to the original degraded face."}),
                "face_restore_edge_blur": ("BOOLEAN", {"default": True, "tooltip": "Apply a soft alpha feathering to the edges of the restored face before pasting to eliminate harsh boundary lines."}),
                "face_restore_blend": ("FLOAT", {"default": 0.5, "min": 0.0, "max": 1.0, "step": 0.05, "tooltip": "Opacity of the restored face. Lower values significantly reduce video flickering by anchoring to the original frame."}),
                
                # --- GROUP: VRAM ---
                "grp_vram_optimization": (["▼ VRAM"], {}),
                "enable_fp16_accumulation": ("BOOLEAN", {"default": False, "tooltip": "Enable torch.backends.cuda.matmul.allow_fp16_accumulation."}),
                "sage_attention": (sageattn_modes, {"default": "disabled", "tooltip": "Patch comfy attention to use sageattn."}),
                "chunks": ("INT", {"default": 4, "min": 1, "max": 100, "step": 1, "tooltip": "Number of chunks to split the feedforward activations into to reduce peak VRAM usage."}),
            },
            "optional": {
